...
df$forum_posts <- as.numeric(df$forum_posts)
df$quiz_attempts <- as.numeric(df$quiz_attempts)

# Standardize predictors within each country (vectorized with ave)
df$GPA_std <- ave(df$GPA, df$country, FUN = function(x) if(length(x) > 1) as.vector(scale(x)) else rep(0, length(x)))
df$forum_posts_std <- ave(df$forum_posts, df$country, FUN = function(x) if(length(x) > 1) as.vector(scale(x)) else rep(0, length(x)))
df$quiz_attempts_std <- ave(df$quiz_attempts, df$country, FUN = function(x) if(length(x) > 1) as.vector(scale(x)) else rep(0, length(x)))

countries <- unique(df$country)

# Function to fit a model for one country
fit_model <- function(c) {
  data_i <- df[df$country == c, ]
  glmer(completion ~ GPA_std + forum_posts_std + quiz_attempts_std + (1 | device),
        data = data_i, family = binomial)
}

# Record running time of entire script
time_total <- system.time({

  # Set up cluster using all available cores minus 1
  ...
  # Name the models by country
  names(models) <- countries
  
  # Extract forum_posts coefficients
